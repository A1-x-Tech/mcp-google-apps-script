...
**В приложении:** откройте **Settings → Plugins → MCP servers**, нажмите **Add server**, затем добавьте `npx -y mcp-google-apps-script@latest` с `GOOGLE_APPS_SCRIPT_CLIENT_ID`, `GOOGLE_APPS_SCRIPT_CLIENT_SECRET` и `GOOGLE_APPS_SCRIPT_REFRESH_TOKEN`.
