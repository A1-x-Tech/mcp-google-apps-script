...
before(() => {
  ...
  // GOOGLE_APPS_SCRIPT_* live and re-reads $XDG_CONFIG_HOME files per call.
  for (const key of [...ENV_KEYS, "XDG_CONFIG_HOME"]) savedEnv.set(key, process.env[key]);
  for (const key of ENV_KEYS) delete process.env[key];
  configDir = mkdtempSync(join(tmpdir(), "mcp-apps-script-auth-test-"));
...
test("options pin this server's wiring: env prefix, server name and documented scopes", () => {
  assert.equal(AUTH_OPTIONS.envPrefix, "GOOGLE_APPS_SCRIPT", "must read the same variables as config.ts");
  assert.equal(AUTH_OPTIONS.serverName, "apps-script", "token path must be mcp-google-apps-script/credentials.json");
...
test("registers the component tools minus setup_instructions, and returns the shared TokenProvider", () => {
  const { tools, server } = fakeServer();
  const provider = registerAuthTools(server);
  assert.deepEqual(
    [...tools.keys()].sort(),
    // `setup_instructions` is deliberately NOT taken from the component: this
    // server ships its own, which also covers the per-account Apps Script API
    // toggle and the run_function prerequisites. Two tools of one name would
    // collide, and the generic text would lose that detail.
    ["auth_status", "finish_login", "logout", "set_client", "start_login"],
  );
...
test("env priority flows through the provider: GOOGLE_APPS_SCRIPT_ACCESS_TOKEN wins (invariant 3)", async () => {
  const { server } = fakeServer();
  const provider = registerAuthTools(server);
  process.env.GOOGLE_APPS_SCRIPT_ACCESS_TOKEN = "ENV-TOK";
  try {
    ...
    delete process.env.GOOGLE_APPS_SCRIPT_ACCESS_TOKEN;
  }
...
test("auth_status routes to the provider: not connected, this server's paths, no error", async () => {
  ...
  assert.match(text, /mcp-google-apps-script/, "must report this server's own credentials path");
});
...
test("the unconfigured prefix offers the in-chat login first and the env path as alternative", () => {
  ...
  assert.match(prefix, /GOOGLE_APPS_SCRIPT_CLIENT_ID/);
  assert.ok(
    prefix.indexOf("start_login") < prefix.indexOf("GOOGLE_APPS_SCRIPT_CLIENT_ID"),
