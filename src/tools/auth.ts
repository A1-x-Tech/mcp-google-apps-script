 ...
 * docs/TOOLS.md). The component always adds the identity scopes on top. Apps Script splits its surface across four scopes; the read-only variants are not requested because the tools create versions and deployments.
 */
...
/**
 * `verifyIdentity` for the component: one cheap Google Apps Script read, plus the
 ...
 * is the point: OIDC answers even when Apps Script API is switched off, so a
...
async function verifyIdentity(accessToken: string): Promise<{ email?: string }> {
  try {
    ...
    if (error instanceof GoogleAppsScriptError && error.status === 403 && isApiDisabled(error.body)) {
      throw new OAuthError("accessNotConfigured", "Apps Script API", error.status);
    }
    throw error;
  }
...
/**
 * The single source of the auth wiring: serverName "apps-script" puts the stored
 * login in ~/.config/mcp-google-apps-script/credentials.json (the component
 * prefixes "mcp-google-"), envPrefix keeps the provider reading the exact
 * GOOGLE_APPS_SCRIPT_* variables config.ts documents — which is what preserves the
 * env-beats-stored priority for existing installs.
 */
export const AUTH_OPTIONS: GoogleAuthOptions = {
  serverName: "apps-script",
  envPrefix: "GOOGLE_APPS_SCRIPT",
  scopes: LOGIN_SCOPES,
  verifyIdentity,
};
...
export function registerAuthTools(server: McpServer): TokenProvider {
  return registerGoogleAuth(skipSetupInstructions(server), AUTH_OPTIONS);
}

/**
 * This server already ships a richer `setup_instructions` (src/tools/setup.ts):
 * besides the OAuth client it covers the per-account Apps Script API toggle at
 * script.google.com/home/usersettings, the per-tool scopes and the extra
 * prerequisites of run_function — none of which the generic component knows
 * about. Registering both would collide on the name, so the component's version
 * is dropped here and the local one carries the login steps instead. Everything
 * else the component registers passes through untouched.
 */
function skipSetupInstructions(server: McpServer): McpServer {
    ...
    get(target, prop, receiver) {
      ...
      return (name: string, config: unknown, handler: unknown) =>
        name === "setup_instructions"
          ? undefined
          : original(name, config as never, handler as never);
    },
  });
}
