...
/** Default Google Apps Script API host. */
export const DEFAULT_BASE = "https://script.googleapis.com";
...
export const MISSING_CREDENTIALS_MESSAGE =
  ...
  "GOOGLE_APPS_SCRIPT_ACCESS_TOKEN with a short-lived access token. " +
...
export function hasCredentials(config: GoogleAppsScriptConfig): boolean {
 ...
 *
 *   GOOGLE_APPS_SCRIPT_CLIENT_ID      OAuth2 client id      (refresh flow, recommended)
 *   GOOGLE_APPS_SCRIPT_CLIENT_SECRET  OAuth2 client secret  (refresh flow)
 *   GOOGLE_APPS_SCRIPT_REFRESH_TOKEN  OAuth2 refresh token  (refresh flow)
 *   GOOGLE_APPS_SCRIPT_ACCESS_TOKEN   static access token (alternative; expires in ~1h)
 *   GOOGLE_APPS_SCRIPT_API_BASE       API root override (default https://script.googleapis.com)
 *   GOOGLE_APPS_SCRIPT_TIMEOUT_MS     per-request timeout (default 60000)
 *   GOOGLE_APPS_SCRIPT_MAX_RETRIES    retries on transient errors (default 3)
 */
export function loadConfig(): GoogleAppsScriptConfig {
  ...
  const accessToken = process.env.GOOGLE_APPS_SCRIPT_ACCESS_TOKEN;

  const oauthProvided = [clientId, clientSecret, refreshToken].filter(Boolean).length;
  if (oauthProvided > 0 && oauthProvided < 3) {
    throw new ConfigError(
      "GOOGLE_APPS_SCRIPT_CLIENT_ID, GOOGLE_APPS_SCRIPT_CLIENT_SECRET and GOOGLE_APPS_SCRIPT_REFRESH_TOKEN must be set together (OAuth2 refresh flow).",
      "incomplete_oauth_config",
    );
  }

  const timeoutMs = Number(process.env.GOOGLE_APPS_SCRIPT_TIMEOUT_MS);
  const maxRetries = Number(process.env.GOOGLE_APPS_SCRIPT_MAX_RETRIES);
  ...
  return {
    ...
    refreshToken,
    accessToken,
    apiBase: process.env.GOOGLE_APPS_SCRIPT_API_BASE || DEFAULT_BASE,
