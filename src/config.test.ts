...
test("no credentials at all is not an error — the config loads with empty fields", () => {
  withEnv({}, () => {
    ...
    assert.equal(config.apiBase, "https://script.googleapis.com");
    assert.equal(hasCredentials(config), false);
  });
});

test("a partial OAuth triple reports incomplete_oauth_config", () => {
  assert.equal(reasonOf({ GOOGLE_APPS_SCRIPT_CLIENT_ID: "id" }), "incomplete_oauth_config");
  assert.equal(
    reasonOf({ GOOGLE_APPS_SCRIPT_CLIENT_ID: "id", GOOGLE_APPS_SCRIPT_CLIENT_SECRET: "secret" }),
  ...
  assert.equal(
    reasonOf({ GOOGLE_APPS_SCRIPT_REFRESH_TOKEN: "rt", GOOGLE_APPS_SCRIPT_ACCESS_TOKEN: "at" }),
    "incomplete_oauth_config",
  );
});
...
test("the full refresh triple loads without throwing", () => {
  withEnv(
    {
      GOOGLE_APPS_SCRIPT_CLIENT_ID: "id",
      GOOGLE_APPS_SCRIPT_CLIENT_SECRET: "secret",
      GOOGLE_APPS_SCRIPT_REFRESH_TOKEN: "rt",
    },
    () => {
      const config = loadConfig();
...
test("a static access token alone is enough", () => {
  withEnv({ GOOGLE_APPS_SCRIPT_ACCESS_TOKEN: "at" }, () => {
    const config = loadConfig();
...
test("invalid numeric overrides fall back to the defaults", () => {
  withEnv(
    {
      GOOGLE_APPS_SCRIPT_ACCESS_TOKEN: "at",
      GOOGLE_APPS_SCRIPT_TIMEOUT_MS: "not-a-number",
      GOOGLE_APPS_SCRIPT_MAX_RETRIES: "-5",
    },
    () => {
      const config = loadConfig();
...
test("numeric overrides are honored when valid", () => {
  withEnv(
    {
      GOOGLE_APPS_SCRIPT_ACCESS_TOKEN: "at",
      GOOGLE_APPS_SCRIPT_TIMEOUT_MS: "1000",
      GOOGLE_APPS_SCRIPT_MAX_RETRIES: "0",
    },
    () => {
      const config = loadConfig();
...
test("the API base override is honored", () => {
  withEnv(
    { GOOGLE_APPS_SCRIPT_ACCESS_TOKEN: "at", GOOGLE_APPS_SCRIPT_API_BASE: "https://example.test" },
    () => {
