...
export class GoogleAppsScriptClient {
  ...
  constructor(
    private readonly config: GoogleAppsScriptConfig,
    /**
  ...
  private async accessToken(forceRefresh = false): Promise<string> {
    if (!this.canRefresh()) {
      ...
      // GOOGLE_APPS_SCRIPT_ACCESS_TOKEN keeps behaving exactly as before.
...
/**
 * One cheap Google Apps Script read, used to verify a fresh in-chat login against the
 ...
 * finished. Throws GoogleAppsScriptError exactly like the client does, so the caller
...
export async function probeApi(accessToken: string): Promise<void> {
  const base = (process.env.GOOGLE_APPS_SCRIPT_API_BASE || DEFAULT_BASE).replace(/\/+$/, "");
  const res = await fetch(`${base}/v1/processes?pageSize=1`, {
  ...
  throw new GoogleAppsScriptError(res.status, data);
}
