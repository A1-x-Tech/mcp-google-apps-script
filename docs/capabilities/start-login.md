# Google Apps Script: Start the login — MCP tool

**Google Apps Script MCP tool:** Opens a loopback + PKCE login and returns the Google consent URL for the user to approve in their browser.

Technical name: `start_login`

## What task it solves

> I want to connect my Google account to Google Apps Script from inside this conversation.
...
> Connect my Google account to Google Apps Script — give me the link to approve.

## Errors and limitations

Deliberately not marked read-only: it opens a local listener, holds pending state and initiates an OAuth flow — a read-only hint would let a client run it without confirmation, so a prompt injection could start a login silently. The attempt expires after ten minutes — start again if the user takes longer. A blocked or busy loopback port breaks the redirect; pin one with `GOOGLE_APPS_SCRIPT_OAUTH_PORT` when the client runs over SSH forwarding. Without an OAuth client the call fails actionably, naming `set_client` and the environment variables.
