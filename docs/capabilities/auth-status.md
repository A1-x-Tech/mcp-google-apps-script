# Google Apps Script: Connection status — MCP tool

**Google Apps Script MCP tool:** Reports whether Google Apps Script is connected, where the token comes from, when it expires and which account it belongs to.

Technical name: `auth_status`

## What task it solves

> I want to check whether this server is connected to Google Apps Script and as whom.

Answers the "why does every tool fail" question before anyone starts editing configuration files: token present or not, its source (the `GOOGLE_APPS_SCRIPT_*` environment variables or a stored in-chat login), its expiry, the account email and the paths of the stored client and credentials.
...
Nothing. The tool reads local connection state and does not touch Google Apps Script.

## Example request

> Show the Google Apps Script connection status — are we connected, and as which account?
