# Google Apps Script: Save the OAuth client — MCP tool

**Google Apps Script MCP tool:** Reads the OAuth client JSON downloaded from Google Cloud Console and stores it for every mcp-google-* server to reuse.
...
Use it right after the user creates a Desktop OAuth client with the steps from [`setup_instructions`](./setup-instructions.md), or when switching to a different Cloud project. Not needed when `GOOGLE_APPS_SCRIPT_CLIENT_ID` and `GOOGLE_APPS_SCRIPT_CLIENT_SECRET` are already set in the environment — those win.
...
Nothing in Google Apps Script. Locally, the shared client file is created or overwritten with owner-only permissions.
